...
export default function Curadoria(props) {
  ...
  const columns = [
    ...
    },
    {
      ...
      field: 'arquivo',
      width: 100,
      defaultFilter: props.filter.arquivo,
    },
    {
      title: 'Tema',
      field: 'tema',
      width: 100,
      initialEditValue: '',
      defaultFilter: props.filter.tema,
    },
    {
      ...
      render: (props) => {
        const perguntas = props?.inputTextArray
          .map((input) => input.inputText)
          .join('\n')
          ?.replace(/\n/g, '<br/>');
        return (
          <React.Fragment>
            {perguntas &&
              parse(
                `${perguntas.substr(0, 200)}${
                  perguntas?.length > 200 ? '...' : ''
                }`
              )}
          </React.Fragment>
        );
    ...
    },
    {
      ...
      render: (props) => {
        const respostas = props?.outputText?.replace(/\n/g, '<br/>');
        return (
          <React.Fragment>
            {respostas &&
              parse(
                `${respostas?.substr(0, 250)}${
                  respostas?.length > 250 ? '...' : ''
                }`
              )}
          </React.Fragment>
        );
    ...
    },
    {
      ...
      field: 'validacaoConteudo',
      width: 50,
      type: 'boolean',
      defaultFilter: props.filter.validacaoConteudo,
    },
    {
      title: 'Possível validar no BOT',
      field: 'possivelValidarBOT',
      width: 50,
      type: 'boolean',
      defaultFilter: props.filter.possivelValidarBOT,
    },
    {
      title: 'Validação BOT',
      field: 'validacaoBOT',
      width: 50,
      type: 'boolean',
      defaultFilter: props.filter.validacaoBOT,
    },
    {
      title: 'Responsável',
      field: 'responsavel',
      defaultFilter: props.filter.responsavel,
      customFilterAndSearch: (term, { responsavel }) =>
        term === '!'
          ? !responsavel
          : responsavel &&
            responsavel.toLowerCase().includes(term.toLowerCase()),
    },
    {
      ...
      type: 'date',
      editable: 'never',
      defaultFilter: props.filter.updatedAt,
  ...
  const handleDelete = (_) => {
    selectedRows.forEach(async (row) => {
      ...
      await api.delete('/curadoria', {
        params: { _id: row._id, bot: row.bot },
      });
    });
  ...
  return (
    <>
      <Editor
        bot={props.botName}
        id={itemSelecionado}
        setIitemSelecionado={setItemSelecionado}
        open={editModalOpen}
        setOpen={setEditModalOpen}
        setCuradorias={props.setCuradorias}
      />
      <Dialog
        name='deleteDialog'
      ...
      <MaterialTable
        ...
        onFilterChange={(columns) => {
          const newFilter = columns.reduce((acc, current) => {
            return { ...acc, [current.column.field]: current.value };
          }, {});
