...
export default function Sugestoes({ surveys, connections, users }) {
  ...
  const [groupedConnectionsByStatus, setGroupedConnectionsByStatus] = useState([]);
  const [conexoesPorDia, setConexoesPorDia] = useState([]);
  const [conexoesPorDesafio, setConexoesPorDesafio] = useState([]);

  const COLORS = ['#9FC439', '#FEC134', '#0088FE', '#00C49F', '#FF8042'];
  ...
  useEffect(() => {
    ...
    setGroupedConnectionsByStatus(grouped);
    ...
    <Container maxWidth='lg' className={classes.container}>
      <Paper className={classes.paper}>
        <Grid container spacing={2}>
          <Grid item xs={2}>
            <Paper>
              <Paper className={classes.title}>Pessoas Cadastradas</Paper>
              <Paper className={classes.description}>{users.length}</Paper>
              <Paper className={classes.title}>Número de conexões</Paper>
              <Paper className={classes.description}>{connections.length}</Paper>
              <Paper className={classes.title}>Nota média da pesquisa</Paper>
              <Paper className={classes.description}>
                {surveys.length &&
                  (
                    surveys
                      .filter((s) => s.nota)
                      .reduce((a, b) => a + (b.nota > 10 ? 10 : b.nota), 0) /
                    surveys.filter((s) => s.nota).length
                  ).toFixed(2)}
              </Paper>
            </Paper>
          </Grid>
          <Grid item xs={6}>
            <Paper>{locais.length && <GenericBarChart data={locais} />}</Paper>
          </Grid>
          <Grid item xs={4}>
            <Paper>
              <ResponsiveContainer width='100%' aspect={1.31}>
                <PieChart width={'100%'} height={'57%'}>
                  <Pie
                    ...
                    dataKey='Quantidade'
                  >
                    {groupedSurveys.map((entry, index) => (
          ...
          <Grid item xs={7}>
            <Paper>
              <ResponsiveContainer aspect={1.35}>
                <LineChart
                  width={400}
                  height={300}
                  data={conexoesPorDia}
            ...
            <Paper>
              <ResponsiveContainer width='100%' aspect={1.8}>
                <PieChart width={'100%'}>
                  <Pie
                    label={renderPieLabel}
                    data={groupedConnectionsByStatus}
                    cx={140}
                    cy={130}
                    innerRadius={50}
                    outerRadius={90}
                    dataKey='Quantidade'
                  >
                    {groupedConnectionsByStatus.map((entry, index) => (
                      <Cell key={index} fill={COLORS[index % COLORS.length]} />
                    ))}
                  </Pie>
                  <Legend layout='vertical' verticalAlign='top' align='right' />
