...
function DataReferenciaSelect(props) {
  ...
  return (
    <FormControl className={props.classes.selects}>
      ...
      >
        {Object.keys(props.cliente?.limits)
          .sort()
          .reverse()
          .map((date) => {
            return (
...
export default function Jira() {
  ...
  const [dataReferencia, setDataReferencia] = useLocalStorageState(
    keys.JIRA_DATA_REFERENCIA,
    '',
    useState
  );
  const [listaClientes, setListaClientes] = useLocalStorageState(
    keys.JIRA_CLIENTES,
    [],
    useState
  );
  const [cliente, setCliente] = useLocalStorageState(
    keys.JIRA_CLIENTE,
    null,
    useState
  );
      ...
      <main className={classes.main}>
        ...
        <Container maxWidth='lg' className={classes.container}>
          <Grid container spacing={3}>
              ...
              <Paper className={classes.dataPickers}>
                ...
                {listaClientes.length && (
                  <div>
                    ...
                    {moment(cliente?.issues[0].updatedAt).format(
                      'DD/MM/YY H:mm:ss'
                    )}
                    <br />
                    <Button
            ...
            <Grid item xs={12}>
              {cliente?.limits[dataReferencia] && (
                <Paper className={classes.chart}>
                  <MainChart
                    cliente={cliente}
                    horas={cliente?.limits}
                    classes={classes}
                  />
                  <Paper className={classes.details}>
                    <div>
                      <b>Consumo de horas no período: </b>
                      {cliente?.limits[dataReferencia]?.used}
                      <br />
                      <b>Pacote de horas: </b>
                      {cliente?.limits[dataReferencia]?.pack}
                      ...
                      >
                        <Fade in={openModal}>
                          <div className={classes.paper}>
                            ...
                            <Button
                              variant='contained'
                              onClick={handleUpdatePacoteHoras}
                            >
                              Salvar
                            </Button>
                          </div>
                      ...
                      {clientEmail ? (
                        ...
                        <b>O client ainda não possui um email cadastrado</b>
                      )}
                      <br />
                      <br />
                      <Button
            ...
            </Grid>
            {
              <Grid item xs={12}>
                <Paper className={classes.table}>
                  <Table
                    issues={cliente?.issues.filter(
                      (issue) =>
                        issue.fields?.dataReferencia
                          ?.toString()
                          .substring(0, 7) === dataReferencia ||
                        (!issue.fields?.dataReferencia &&
